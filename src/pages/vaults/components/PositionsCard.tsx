...
const SynthListItem: React.FC<PositionListItemProps> = ({
	vault,
	vaultName,
	supplyBalances,
	borrowBalances,
...
const CollateralListItem: React.FC<PositionListItemProps> = ({
	vault,
	vaultName,
	supplyBalances,
	borrowBalances,
			...
			<div className='glassmorphic-card my-4 p-2 lg:p-4'>
				<div className='grid w-full grid-cols-12 items-center justify-center px-2'>
					...
					<div className='col-span-3 mr-0 items-start'>
						...
						>
							<Typography variant='lg' className='text-center font-bakbak leading-5'>
								<span className='align-middle'>{`${getDisplayBalance(suppliedUnderlying, vault.underlyingDecimals)}`}</span>
							</Typography>
						</Tooltipped>
					</div>
					<div className='col-span-3 m-auto items-center justify-center'>
						<Typography variant='lg' className={`font-bakbak text-lg leading-5 ${avgBasketAPY > 0 && `text-green`}`}>
							{vault.isBasket && avgBasketAPY
								? getDisplayBalance(avgBasketAPY, 0, 2) + '%'
								: vault.isSynth
								? getDisplayBalance(vault.borrowApy, 18, 2) + '%'
